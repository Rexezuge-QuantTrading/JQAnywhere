...
class Broker(ABC):
    ...
    def order(self, context: Context, security: str, amount: int, **kwargs) -> Order | None:
        raise NotImplementedError("JQAnywhere v0.5 only provides order via broker templates unless implemented by the selected broker")

    def order_target(self, context: Context, security: str, amount: int, **kwargs) -> Order | None:
        raise NotImplementedError(
            "JQAnywhere v0.5 only provides order_target via broker templates unless implemented by the selected broker"
        )
