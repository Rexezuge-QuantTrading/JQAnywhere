...
class PaperBroker(Broker):
    def sync_portfolio(self, context: Context) -> None:
        for pindex, portfolio in enumerate(_subportfolios(context)):
            for security, position in list(portfolio.positions.items()):
                position.pindex = pindex
    ...
    def order_target_value(self, context: Context, security: str, value: float, **kwargs) -> Order | None:
        ...
        portfolio = _portfolio_for_order(context, kwargs)
        ...
        if _blocked_by_price_limit(current_data, execution_price, delta_amount):
            order = _rejected_order(context, security, delta_amount, 0.0, execution_price, "price_limit", _pindex(kwargs))
            _record_order(context, order)
            return None
        ...
        if delta_amount > 0:
            max_affordable = int(portfolio.available_cash / execution_price) if execution_price > 0 else 0
            while (
                max_affordable > 0
            ...
            if delta_amount > max_affordable:
                ...
                commission = _commission(security, trade_value, delta_amount, kwargs)
            if delta_amount <= 0:
                order = _rejected_order(context, security, 0, 0.0, execution_price, "insufficient_cash", _pindex(kwargs))
                _record_order(context, order)
                return None
            ...
            if sell_amount <= 0:
                order = _rejected_order(context, security, delta_amount, 0.0, execution_price, "insufficient_position", _pindex(kwargs))
                _record_order(context, order)
                return None
    ...
    def order(self, context: Context, security: str, amount: int, **kwargs) -> Order | None:
        current = _portfolio_for_order(context, kwargs).positions.get(security)
        current_amount = current.total_amount if current else 0
...
def _rejected_order(context: Context, security: str, amount: int, value: float, price: float, reason: str, pindex: int) -> Order:
    return Order(
        security=security,
        amount=amount,
        value=value,
    ...
    context.order_history.append(
        {
            ...
            "side": order.side,
            "action": order.action,
            "pindex": _pindex({"pindex": getattr(order, "pindex", 0)}),
        }
    )


def _subportfolios(context: Context) -> list[Portfolio]:
    return context.subportfolios or [context.portfolio]


def _pindex(kwargs) -> int:
    return int(kwargs.get("pindex", 0) or 0)


def _portfolio_for_order(context: Context, kwargs) -> Portfolio:
    subportfolios = _subportfolios(context)
    pindex = _pindex(kwargs)
    try:
        return subportfolios[pindex]
    except IndexError as exc:
        raise ValueError(f"invalid subportfolio index: {pindex}") from exc


def _sync_aggregate_portfolio(context: Context) -> None:
    subportfolios = _subportfolios(context)
    context.subportfolios = subportfolios
